...
function AppContent() {
  ...
  useEffect(() => {
    const handleInvitedRedirect = async () => {
       if (user && isInvited && mode !== AppMode.ADMIN_DASHBOARD) {
         // Force redirect to Set Password
         if (!window.location.hash.startsWith('#/set-password')) {
            console.log("User is invited but active. Forcing set password...");
            window.location.hash = '#/set-password';
         }
       }
    };
    handleInvitedRedirect();
  }, [user, isInvited, mode]);
